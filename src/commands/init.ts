...
export const initCommand = new Command("init")
  ...
  .action(async (options: { yes?: boolean; mode?: string; agent?: string; ecosystem?: string; scripts?: string; profile?: string; harness?: string; harnesses?: string; output?: string; headless?: boolean; delivery?: string }) => {
    const detection = detectProject();
    const harnessDetection = detectHarnesses();

    const detectedHarnesses = harnessDetection.filter(h => h.detected);

    const parseHarnesses = (input: string): HarnessName[] => {
    ...
    if (options.headless) {
      ...
      const ecosystem: Ecosystem = (VALID_ECOSYSTEMS.includes(options.ecosystem as Ecosystem)
        ? options.ecosystem : detection.ecosystem) as Ecosystem;
      const profile: TokenProfile = (VALID_PROFILES.includes(options.profile as TokenProfile)
        ? options.profile : "medium") as TokenProfile;

      let selectedIds: string[];
      if (options.scripts) {
        selectedIds = options.scripts.split(",").map((s) => s.trim()).filter(Boolean);
      } else {
        selectedIds = detection.recommendedScripts;
      }
      ...
      if (pmdMode === "file") {
        ...
        console.log("PIPEDM_HEADLESS_RESULT=" + JSON.stringify(summary));
        return;
      }

      let harnesses: HarnessName[];
      if (options.harnesses) {
        harnesses = parseHarnesses(options.harnesses);
      } else if (detectedHarnesses.length > 0) {
        harnesses = detectedHarnesses.map(h => h.name);
      } else {
        harnesses = ["OS Agent"];
      }
      ...
      const agent: AiAgent = harnesses.includes("Claude Code") ? "Claude Code"
      ...
      const validDeliveryModes: DeliveryMode[] = ["passive", "active", "expert"];
      const delivery: DeliveryMode = validDeliveryModes.includes(options.delivery as DeliveryMode)
        ? (options.delivery as DeliveryMode)
        : "active";

      runInit(agent, ecosystem, selectedIds, profile, harnesses, delivery);
      installHooksForHarnesses(harnesses, delivery);

      const targetFiles = harnesses.map(h => HARNESS_TARGETS[h]);
      const summary = {
        status: "ok",
        mode: pmdMode,
        delivery,
    ...
    if (options.yes) {
      ...
      const harness: Harness = (VALID_HARNESSES.includes(options.harness as Harness)
        ? options.harness : "None") as Harness;
      const agent: AiAgent = (VALID_AGENTS.includes(options.agent as AiAgent)
        ? options.agent : "Generic") as AiAgent;
      ...
      if (detectedHarnesses.length > 0) {
        console.log(chalk.green("✅ Detected harnesses: " + detectedHarnesses.map(h => h.name).join(", ")));
...
░░░░░        ░░░░░  ░███░░░   ░░░░░░  ░░░░░     ░░░░░ ░░░░░░░░░░   
    ...
    if (selectedMode === "agent") {
      const harnessChoices = ALL_HARNESSES.map((name) => ({
        ...
        value: name,
        selected: detectedHarnesses.some(h => h.name === name),
      }));
      ...
      if (selectedHarnesses.includes("OpenClaw") && selectedHarnesses.includes("Hermes")) {
        console.log(chalk.yellow(
          "  ⚠ OpenClaw and Hermes both target WORKSPACE_CONTEXT.md. " +
          "Only one pipe will serve the file — the last one registered wins."
        ));
      ...
      if (!options.delivery) {
        ...
        const validDeliveryModes: DeliveryMode[] = ["passive", "active", "expert"];
        interactiveDelivery = validDeliveryModes.includes(options.delivery as DeliveryMode)
          ? (options.delivery as DeliveryMode)
          : "active";
      }
    } else {
    ...
    if (selectedMode === "agent") {
      ...
      if (!harnessPromptAnswer.harness) {
        console.log(chalk.yellow("\nSetup cancelled."));
        process.exit(1);
      }
      ...
      const agent: AiAgent = selectedHarnesses.length > 0
        ? (selectedHarnesses.includes("Claude Code") ? "Claude Code"
          : selectedHarnesses.includes("Gemini") ? "Gemini"
          : selectedHarnesses.includes("Cursor") ? "Cursor"
          : selectedHarnesses.includes("Aider") ? "Aider"
          : selectedHarnesses.includes("OpenClaw") ? "OpenClaw"
          : selectedHarnesses.includes("Hermes") ? "Hermes"
          : "Generic")
        : "Generic";
      ...
      if (selectedHarnesses.length > 0) {
        printHarnessRouting(selectedHarnesses);
      }
