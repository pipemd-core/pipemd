#!/usr/bin/env bash
# limit-core.sh — Shared token profile system and output limiting functions.
# Sourced by ecosystem-specific limit.sh files which override TREE_EXCLUDES
# and TREE_FIND_EXCLUDES before sourcing this file.
set -uo pipefail
...
if [ "${PMD_UNLIMITED:-0}" = "1" ]; then
  ...
  MAX_EXPORTS=${PMD_MAX_EXPORTS:-99999}
  MAX_WORKSPACE_MAP=${PMD_MAX_WORKSPACE_MAP:-99999}
  MAX_DJANGO_URLS=${PMD_MAX_DJANGO_URLS:-99999}
  MAX_HOTSPOTS=${PMD_MAX_HOTSPOTS:-99999}
  MAX_NOW=${PMD_MAX_NOW:-99999}
  MAX_DEADCODE=${PMD_MAX_DEADCODE:-99999}
  MAX_GIT_CONTEXT=${PMD_MAX_GIT_CONTEXT:-99999}
  MAX_REPOMAP=${PMD_MAX_REPOMAP:-99999}
 else
  MAX_TREE=$(( (${PMD_MAX_TREE:-80} * MULT_NUM) / MULT_DEN ))
  ...
  MAX_EXPORTS=$(( (${PMD_MAX_EXPORTS:-30} * MULT_NUM) / MULT_DEN ))
  MAX_WORKSPACE_MAP=$(( (${PMD_MAX_WORKSPACE_MAP:-60} * MULT_NUM) / MULT_DEN ))
  MAX_DJANGO_URLS=$(( (${PMD_MAX_DJANGO_URLS:-40} * MULT_NUM) / MULT_DEN ))
  MAX_HOTSPOTS=$(( (${PMD_MAX_HOTSPOTS:-15} * MULT_NUM) / MULT_DEN ))
  MAX_NOW=$(( (${PMD_MAX_NOW:-1} * MULT_NUM) / MULT_DEN ))
  MAX_DEADCODE=$(( (${PMD_MAX_DEADCODE:-30} * MULT_NUM) / MULT_DEN ))
  MAX_GIT_CONTEXT=$(( (${PMD_MAX_GIT_CONTEXT:-35} * MULT_NUM) / MULT_DEN ))
  MAX_REPOMAP=$(( (${PMD_MAX_REPOMAP:-60} * MULT_NUM) / MULT_DEN ))
 fi
...
# limit_tree() — uses TREE_EXCLUDES for `tree -I` and TREE_FIND_EXCLUDES for
# the `find` fallback. Ecosystem-specific limit.sh files set these before
# sourcing this file. Default: exclude .git and .pipemd only.
TREE_EXCLUDES="${PMD_TREE_EXCLUDES:-${TREE_EXCLUDES:-.git|.pipemd}}"
TREE_FIND_EXCLUDES="${TREE_FIND_EXCLUDES:- \
  -not -path '*/.git/*' \
  -not -path '*/.pipemd/*' \
  -not -name '.git' \
  -not -name '.pipemd'}"
