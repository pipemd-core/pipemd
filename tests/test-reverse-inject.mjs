...
const EMPTY_BLOCK = (name) =>
  `<!-- pmd: ${name} -->\n\`\`\`\n\n\`\`\`\n<!-- /pmd -->`;

// Mirrors src/core/injector.ts:reverseInject. If the implementation changes,
// update this copy. Cannot import directly — test:unit runs without a build.
function reverseInject(renderedContent, template) {
  ...
  const result = renderedContent.replace(blockRe, (_match, commandName) => {
